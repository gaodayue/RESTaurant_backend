...
router.post('/update/:DISHID', function(req, res) {
  ...
  if(!dishes.r_id)
    ...
    async.waterfall([
      function(callback){
        var result;
        var query = connection.query('SELECT d_id, d_name, d_price FROM dishes WHERE d_id = ? AND d_rest_id = ?', [dishId, dishes.r_id], function(err, row){
          if(err)
            callback('error');
          else {
...
router.post('/delete/:DISHID', function(req, res) {
  ...
  if (!restId)
    ...
    res.json(400, util.showError('missing dish id'));
  else {
    async.series([
      function(callback){
        var result;
